...
export default function AdminSectionsPage() {
    ...
    try {
      if (editingId) {
        ...
        const insertData = {
          ...
          slug: formData.slug,
          description: formData.description,
          order_index: sections.filter(s => s.category_id === formData.category_id).length,
        };
        const { error } = await (supabase.from('sections') as any).insert(insertData);
      ...
      toast.error('Error al guardar la sección: ' + error.message);
    }
  };

  const deleteSection = async (id: string) => {
    if (!confirm('¿Estás seguro? Esto también eliminará todas las entradas relacionadas.')) return;
  ...
  return (
    <AdminLayout>
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-6">Gestión de Secciones</h1>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-card divide-y divide-gray-200">
              {sections.map((section) => (
                <tr key={section.id} className="hover:bg-muted">
                  <td className="px-6 py-4">
                    <span className="text-sm font-medium text-foreground">{section.name}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-muted-foreground">{getCategoryName(section.category_id)}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-muted-foreground">{section.slug}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-muted-foreground">{section.order_index}</span>
                  </td>
