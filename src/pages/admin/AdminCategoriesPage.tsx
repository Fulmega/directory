...
export default function AdminCategoriesPage() {
  ...
  const deleteCategory = async (id: string) => {
    if (!confirm('¿Estás seguro? Esto también eliminará todas las secciones y entradas relacionadas.')) return;
  ...
  return (
    <AdminLayout>
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-6">Gestión de Categorías</h1>
            ...
            <tbody className="bg-card divide-y divide-gray-200">
              {categories.map((category) => (
                <tr key={category.id} className="hover:bg-muted">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-muted-foreground">{category.order_index}</span>
                  </td>
